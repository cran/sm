...
rr <- ratio/(1+ratio)
par(cex=0.8)
persp(xgrid, ygrid, rr, zlim=c(0,0.8), theta=-30, phi=30, d=4)
#	xlab="Easting", ylab="Northing", zlab="Risk")
persp(xgrid, ygrid, log(rr), zlim=c(-1.5,0), theta=-30, phi=30, d=4)
#	xlab="Easting", ylab="Northing", zlab="Log risk")
diff.sm <- cases.sm$estimate - controls.sm$estimate
se      <- sqrt(cases.sm$se^2 + controls.sm$se^2)
persp(xgrid, ygrid, diff.sm/se, theta=-30, phi=30, d=4)
